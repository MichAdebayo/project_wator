...
class Shark:
    ...
    def __init__(self, energy, position, grille, compteur_tour=1):
    ...
    def check_and_move(self):
        ...
        east_position = ((self.position[0]) % len(self.grille), (self.position[1]+1) % len(self.grille))
        west_position = ((self.position[0] )% len(self.grille), (self.position[1]-1 )% len(self.grille))
        north_position = ((self.position[0]-1 )% len(self.grille), (self.position[1])% len(self.grille))
        south_position = ((self.position[0]+1) % len(self.grille), (self.position[1]) % len(self.grille))
        ...
        for i in position_voisine:
            ...
            if 0 <= i[0] < len(self.grille) and 0 <= i[1] < len(self.grille[0]):  # limites de la grille
                controle_case = self.grille[i[0]][i[1]]
        ...
        if thon_possible: 
            ...
            self.position = eat
            self.energy += 1
            self.grille[eat[0]][eat[1]] = "S"
            self.grille[self.ancienne_position[0]][self.ancienne_position[1]] = "."

        elif mouv_possible:
            mouv = random.choice(mouv_possible) 
            self.position = mouv
            self.energy -= 1
            self.grille[mouv[0]][mouv[1]] = "S"
            self.grille[self.ancienne_position[0]][self.ancienne_position[1]] = "."

    # def reproduce(self):
    #     if self.compteur_tour == 5:
    #         self.grille[self.ancienne_position[0]][self.ancienne_position[1]] = "S" 
    #         self.grille[shark.position[0]][shark.position[1]] = "S"
    #         self.compteur_tour = 0

    def reproduce(self):
        if self.compteur_tour == 5:
            new_shark = Shark(energy=10, position=(self.ancienne_position[0],self.ancienne_position[1]))
            self.grille[self.ancienne_position[0]][self.ancienne_position[1]] = new_shark
            self.grille[self.position[0]][self.position[1]] = "S" 
            self.compteur_tour = 0


    def check_energy(self):
        if self.energy == 0:
            self.grille[self.position[0]][self.position[1]] = "." #si requin n'a plus d'energie il est remplacé par une case vide (cad mort)
